...
const PFCalculator: React.FC<PFCalculatorProps> = ({
  ...
  const { ecrData, challan } = useMemo(() => {
    ...
    const rows = currentRecords.map(r => {
      ...
      // Higher Pension Check
      const epsWage = (emp.pfHigherPension?.enabled && emp.pfHigherPension.isHigherPensionOpted === 'Yes') 
          ? actualWage 
          : Math.min(actualWage, 15000); // Standard Cap 15k
      const edliWage = Math.min(actualWage, 15000); // EDLI always capped at 15k
      ...
      ac1_EE += (r.deductions.epf + r.deductions.vpf); // EE Share includes VPF
      ...
      return {
        ...
        eeShare: Math.round(r.deductions.epf + r.deductions.vpf),
        erEPF: Math.round(r.employerContributions.epf),
        erEPS: Math.round(r.employerContributions.eps),
        ncp: r.daysInMonth - r.payableDays
      };
    }).filter(Boolean) as any[];

    // Final Challan Calculations
    // A/c 2 (Admin) = 0.5% of EPF Wages (Min 500)
    const adminCharges = Math.round(totalEPFWages * 0.005);
    ac2 = Math.max(500, adminCharges); 
    
    // A/c 21 (EDLI) = 0.5% of EDLI Wages
    ac21 = Math.round(totalEDLIWages * 0.005);

    // Total Remittance
    const totalRemittance = ac1_EE + ac1_ER + ac10 + ac2 + ac21; // A/c 22 is 0
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-top-4 duration-500">
      ...
      <div className="bg-[#1e293b] p-6 rounded-xl border border-slate-800 flex flex-col md:flex-row items-center justify-between gap-6 shadow-xl">
        ...
        <div className="flex items-center gap-3 bg-[#0f172a] p-2 rounded-xl border border-slate-700">
             <select 
                ...
                className="bg-transparent border-r border-slate-700 px-4 py-1 text-sm text-white font-bold outline-none focus:text-blue-400"
             >
      ...
      {currentRecords.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center p-16 bg-[#1e293b] rounded-2xl border border-slate-800 border-dashed text-center">
              <div className="p-4 bg-amber-900/20 text-amber-500 rounded-full border border-amber-900/50 mb-4">
                  <Lock size={40} />
              </div>
              ...
              <p className="text-slate-400 mt-2 max-w-lg leading-relaxed">
                  PF ECR generation requires the payroll for <span className="text-blue-400 font-bold">{month} {year}</span> to be permanently frozen. 
                  This ensures consistency between the ECR filing and your ledgers.
              </p>
              <div className="mt-6 p-4 bg-[#0f172a] rounded-xl border border-slate-700 text-sm text-slate-300">
                  Go to <span className="font-bold text-white">Analytics &gt; Pay Reports</span> and click <span className="font-bold text-emerald-400">Confirm & Freeze Data</span>.
              </div>
          </div>
      ) : (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-[#1e293b] p-6 rounded-2xl border border-slate-800 shadow-lg relative overflow-hidden group">
                  ...
                  <div className="mt-4 flex items-center gap-2 text-[10px] text-emerald-300 bg-emerald-900/20 px-2 py-1 rounded w-fit">
                      <span>A/c 1 + 2 + 10 + 21 + 22</span>
                  </div>
              </div>
              ...
              <div className="bg-[#1e293b] p-6 rounded-2xl border border-slate-800 shadow-lg relative overflow-hidden group">
                  ...
                  <h3 className="text-3xl font-black text-blue-400">{ecrData.length}</h3>
                  <p className="text-xs text-slate-400 mt-1">Employees Contributing</p>
              </div>
              ...
              <div className="bg-[#1e293b] p-6 rounded-2xl border border-slate-800 shadow-lg relative overflow-hidden group">
                  ...
                  <div className="mt-4 text-[10px] text-slate-400">
                      EPS Wages: <span className="font-bold text-white">₹{challan.totalEPSWages.toLocaleString()}</span>
                  </div>
              </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                          ...
                          <tbody className="divide-y divide-slate-800 text-slate-300">
                              <tr>
                                  <td className="px-6 py-3">
                                      <span className="font-bold text-white">A/c No. 1</span> <span className="text-xs text-slate-500 ml-2">PF Contribution (EE + ER Diff)</span>
                                  </td>
                                  <td className="px-6 py-3 text-right font-mono font-bold text-white">{challan.ac1_Total.toLocaleString()}</td>
                              </tr>
                              <tr>
                                  <td className="px-6 py-3">
                                      <span className="font-bold text-white">A/c No. 2</span> <span className="text-xs text-slate-500 ml-2">Admin Charges (0.5%, Min 500)</span>
                                  </td>
                                  <td className="px-6 py-3 text-right font-mono text-amber-400">{challan.ac2.toLocaleString()}</td>
                              </tr>
                              <tr>
                                  <td className="px-6 py-3">
                                      <span className="font-bold text-white">A/c No. 10</span> <span className="text-xs text-slate-500 ml-2">Pension Fund (8.33%)</span>
                                  </td>
                                  <td className="px-6 py-3 text-right font-mono text-white">{challan.ac10.toLocaleString()}</td>
                              </tr>
                              <tr>
                                  <td className="px-6 py-3">
                                      <span className="font-bold text-white">A/c No. 21</span> <span className="text-xs text-slate-500 ml-2">EDLI Contribution (0.5%)</span>
                                  </td>
                                  <td className="px-6 py-3 text-right font-mono text-white">{challan.ac21.toLocaleString()}</td>
                              </tr>
                              <tr>
                                  <td className="px-6 py-3">
                                      <span className="font-bold text-white">A/c No. 22</span> <span className="text-xs text-slate-500 ml-2">EDLI Admin Charges</span>
                                  </td>
                                  <td className="px-6 py-3 text-right font-mono text-slate-500">{challan.ac22.toLocaleString()}</td>
                              </tr>
                              <tr className="bg-slate-900/80">
                                  <td className="px-6 py-4 font-black text-emerald-400 uppercase tracking-widest text-xs">Total Remittance</td>
                                  <td className="px-6 py-4 text-right font-black text-emerald-400 font-mono text-lg">₹{challan.totalRemittance.toLocaleString()}</td>
                              </tr>
                          </tbody>
                      </table>
                  </div>
              ...
              <div className="bg-[#1e293b] rounded-2xl border border-slate-800 shadow-lg p-6 flex flex-col justify-center gap-4">
                  ...
                  <button 
                    ...
                    className="w-full py-4 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold shadow-lg transition-all flex items-center justify-center gap-3 group"
                  >
                      <Download className="group-hover:scale-110 transition-transform" /> 
                      Generate ECR Text File
                  </button>
                  
                  <div className="bg-blue-900/20 p-4 rounded-xl border border-blue-900/50 text-xs text-blue-300 leading-relaxed">
                      <p className="flex gap-2"><AlertCircle size={14} className="shrink-0 mt-0.5" /> <span>Ensure <b>UAN</b> is updated for all employees in Master before generation.</span></p>
                  </div>
              </div>
          </div>
          ...
          <div className="bg-[#1e293b] rounded-2xl border border-slate-800 shadow-lg overflow-hidden">
              <div className="bg-[#0f172a] p-4 border-b border-slate-800">
                  <h3 className="font-bold text-white text-sm flex items-center gap-2">
                      <FileSpreadsheet size={16} className="text-slate-400" /> ECR Data Preview
                  </h3>
              </div>
                  ...
                  <table className="w-full text-left text-xs">
                      <thead className="bg-[#0f172a] text-slate-400 uppercase font-bold sticky top-0 z-10 shadow-md">
                          <tr>
                              ...
                              <th className="px-4 py-3 text-right whitespace-nowrap text-blue-400">EPF Wages</th>
                              <th className="px-4 py-3 text-right whitespace-nowrap text-amber-400">EPS Wages</th>
                              <th className="px-4 py-3 text-right whitespace-nowrap text-pink-400">EDLI Wages</th>
                              <th className="px-4 py-3 text-right whitespace-nowrap">EE Share</th>
                              <th className="px-4 py-3 text-right whitespace-nowrap">ER EPF</th>
                              <th className="px-4 py-3 text-right whitespace-nowrap">ER EPS</th>
                              <th className="px-4 py-3 text-center whitespace-nowrap">NCP</th>
                          </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-800">
                          {ecrData.map((row, i) => (
                              <tr key={i} className="hover:bg-slate-800/50 transition-colors">
                                  ...
                                  <td className="px-4 py-3 text-right font-mono text-slate-300">{row.erEPF}</td>
                                  <td className="px-4 py-3 text-right font-mono text-slate-300">{row.erEPS}</td>
