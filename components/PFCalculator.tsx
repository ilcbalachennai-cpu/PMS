...
const PFCalculator: React.FC<PFCalculatorProps> = ({
    ...
    const { ecrData, challan } = useMemo(() => {
        ...
        const rows = currentRecords.map(r => {
            ...
            // ── Exclude ON LOP employees (all-zero in ECR) ──────────────────────
            ...
            return {
                ...
                eeShare: eeEPF,
                erShareEPS: erEPS,
                erShareEPFDiff: erEPF,
                ncp: Math.max(0, (r.daysInMonth || 30) - Math.round(r.payableDays)),
                refund: 0,
                isOptOut: false,
            };
        }).filter(Boolean) as any[];

        // Admin Charges: 0.5% of EPF Wages (no Min-500 floor — matches reportService)
        ac2 = Math.round(totalEPFWages * 0.005);
