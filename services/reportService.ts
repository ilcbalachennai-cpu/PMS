...
export const generateEPFCodeImpactReport = (records: PayrollResult[], employees: Employee[], format: 'PDF'|'Excel', fileName: string, companyProfile: CompanyProfile) => {
    ...
    if (format === 'Excel') {
        ...
        // Add Summary Row
        // Ensure property access is safe.
        const totalOld = impactData.reduce((acc, curr) => acc + (curr ? curr.oldContrib : 0), 0);
        const totalNew = impactData.reduce((acc, curr) => acc + (curr ? curr.newContrib : 0), 0);
        const totalDiff = totalOld - totalNew;
        rows.push(['TOTAL', '', '', '', '', totalOld.toString(), totalNew.toString(), totalDiff.toString(), totalDiff < 0 ? 'Excess Cost' : 'Saved']);
...
export const generateESIExitReport = (records: PayrollResult[], employees: Employee[], month: string, year: number, companyProfile: CompanyProfile) => {
    ...
    const periodStartStr = formatDateInd(periodStart.toISOString()); // e.g. 01-04-2025

    const data: any[] = records.map(r => {
        const emp = employees.find(e => e.id === r.employeeId);
        if (!emp) return null;
        ...
        if (emp.dol) {
            ...
            if (dolDate.getMonth() === monthIdx && dolDate.getFullYear() === year) {
                reason = 'Left Service';
                dol = formatDateInd(emp.dol);
            }
        }

        if (!reason && (r.earnings?.total || 0) > 21000) {
            reason = 'Out of Coverage';
            
            // Calculate Next Contribution Cycle Start Date based on ESI Cycles: Apr-Sep and Oct-Mar
            let oocYear = year;
            let oocMonth = '04'; // Default Month

            if (monthIdx >= 3 && monthIdx <= 8) { 
                // Apr(3) to Sep(8) -> Currently in Apr-Sep cycle.
                // Next cycle starts Oct 1st of same year
                oocMonth = '10';
                oocYear = year;
            } else if (monthIdx >= 9) {
                // Oct(9) to Dec(11) -> Currently in Oct-Mar cycle (Part 1).
                // Next cycle starts Apr 1st of NEXT year
                oocMonth = '04';
                oocYear = year + 1;
            } else {
                // Jan(0) to Mar(2) -> Currently in Oct-Mar cycle (Part 2).
                // Next cycle starts Apr 1st of CURRENT year
                oocMonth = '04';
                oocYear = year;
            }
            
            oocDate = `01-${oocMonth}-${oocYear}`;
        }

        if (reason) {
            return {
                'EMP_ID': r.employeeId,
                'IP_ESI_No': emp.esiNumber || 'N/A',
                'Emp_Name': emp.name,
                'Reason': reason,
                'DOL': dol,
                '*OoC_Date': oocDate
    ...
    if (data.length > 0) {
        data.push({}); // Spacer
        data.push({ 'EMP_ID': '*Out of Coverage' });
    }
...
export const generateSimplePaySheetPDF = (results: PayrollResult[], employees: Employee[], month: string, year: number, companyProfile: CompanyProfile) => {
    ...
    // Right align numeric columns (Indices 2 to 14)
...
export const generateBankStatementPDF = (results: PayrollResult[], employees: Employee[], month: string, year: number, companyProfile: CompanyProfile) => {
    ...
    const data = results.filter(r => r.netPay > 0).map(r => {
        ...
        return [
            ...
            emp?.bankAccount || '', 
            emp?.ifsc || '', 
            Math.round(r.netPay || 0).toString()
        ];
    });
...
export const generateLeaveLedgerReport = (
    ...
    generatePDFTableReport(
        `Leave Ledger (${type}) ${month} ${year}`, 
        headers, 
        data, 
        `LeaveLedger_${type}_${month}_${year}`, 
        'l', 
        undefined, 
        companyProfile,
        { columnStyles }
