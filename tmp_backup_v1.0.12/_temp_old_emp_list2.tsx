...
const EmployeeList: React.FC<EmployeeListProps> = ({ employees, setEmployees, onAddEmployee, onBulkAddEmployees, designations, divisions, branches, sites, currentUser, companyProfile }) => {
    ...
    const handleDocumentUpload = (e: React.ChangeEvent<HTMLInputElement>, docType: keyof Required<Employee>['employeeDocuments']) => {
        const file = e.target.files?.[0];
        if (!file) return;

        // Restriction: Max 2MB File Size
        if (file.size > 2 * 1024 * 1024) {
            alert('File size exceeds limits. Maximum size is 2MB per document.');
            e.target.value = '';
            return;
        }
    ...
    const proceedToEdit = (emp: Employee, forceUnlock: boolean = false, isRejoin: boolean = false) => {
        setEditingId(emp.id);
        setNewEmpForm({
            ...getEmptyForm(),
            ...emp,
        ...
        if (isRejoin) {
            setTimeout(() => {
                const reasonInput = document.getElementById('leavingReasonInput');
                if (reasonInput) {
    ...
    const handleAuthSubmit = () => {
        if (authModal.password === currentUser?.password && (currentUser?.role === 'Developer' || currentUser?.role === 'Administrator')) {
            if (authModal.mode === 'REJOIN' && authModal.targetEmp) {
                ...
                // Pass true to force unlock separation fields for rejoining, and true for isRejoin
                proceedToEdit(authModal.targetEmp, true, true);
        ...
        <div className="space-y-6 text-white relative">
                        ...
                        <tbody className="divide-y divide-slate-800">
                            {filteredEmployees.map((emp) => (
                                <tr key={emp.id} onClick={() => setSelectedEmp(emp)} className={`cursor-pointer hover:bg-slate-800/50 ${selectedEmp?.id === emp.id ? 'bg-blue-900/40 border-l-4 border-blue-500' : 'border-l-4 border-transparent'} ${emp.dol ? 'opacity-60 grayscale' : ''}`}>
                                    ...
                                    <td className="px-2 py-3 text-center">
                                        <div className="font-mono text-emerald-400 font-bold text-xs">Γé╣{calculateGrossWage(emp).toLocaleString()}</div>
                                    </td>
                        ...
                        <form onSubmit={handleAddSubmit} className="p-8 space-y-12">
                                                            ...
                                                            {hasDoc && (
                                                                <>
                                                                    <button type="button" onClick={() => {
                                                                        setPreviewDoc({
                                                                            url: hasDoc,
                                                                            name: `${newEmpForm.name || 'Employee'} - ${docType.label}`
                                                                        });
                            ...
                            <div>
                                ...
                                <div className="mt-4 p-4 bg-emerald-900/10 border border-emerald-500/20 rounded-xl flex justify-between items-center">
                                    ...
                                    <span className="text-2xl font-black text-white font-mono">Γé╣{calculateGrossWage(newEmpForm).toLocaleString()}</span>
                                </div>
                            </div>
            ...
            {previewDoc && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
                    <div className="bg-[#1e293b] w-full max-w-4xl max-h-[95vh] flex flex-col rounded-3xl border border-slate-700 shadow-2xl overflow-hidden">
                        <div className="p-4 bg-[#0f172a] border-b border-slate-700 flex justify-between items-center">
                            <h3 className="text-lg font-bold text-white flex items-center gap-2">
                                <FileText size={20} className="text-blue-400" /> Document Preview: <span className="text-slate-400 font-normal">{previewDoc.name}</span>
                            </h3>
                            <button onClick={() => setPreviewDoc(null)} className="text-slate-400 hover:text-white bg-slate-800 p-2 rounded-full transition-colors"><X size={20} /></button>
                        </div>
