...
function createWindow() {
    ...
    electron_1.ipcMain.handle('hard-reset-app', async () => {
        try {
    ...
    electron_1.ipcMain.handle('close-update-message', () => {
        try {
...
if (!gotTheLock) {
    ...
    electron_1.app.on('ready', () => {
        const errorWindow = new electron_1.BrowserWindow({
            width: 480,
            height: 280,
            frame: false,
            transparent: true,
...
electron_1.app.whenReady().then(() => {
    ...
    try {
        (0, child_process_1.execSync)('taskkill /F /IM wscript.exe /T', { stdio: 'ignore', windowsHide: true });
    }
    catch (e) { }
...
electron_1.ipcMain.handle('save-report', async (_, { fileName, data, type, subfolder }) => {
    try {
        ...
        if (subfolder) {
            ...
            const segments = subfolder.split(/[/\\]/).map((seg) => {
    ...
    try {
        ...
        for (const siloId of missingSilos) {
            try {
                ...
                const siloDb = new better_sqlite3_1.default(siloDbPath);
                let profileRow = siloDb.prepare(`SELECT value FROM store WHERE key = ?`).get(`app_company_profile_${siloId}`);
                if (!profileRow) {
...
async function performLegacyAutoMigration(rootDb, appPaths) {
    try {
        console.log("🚀 [AutoMigrate] Checking for legacy data in root DB...");
        // 1. Get all keys in root database
        const rows = rootDb.prepare('SELECT * FROM store').all();
        ...
        if (appBasePath) {
            ...
            const globalDb = new better_sqlite3_1.default(globalDbPath);
...
function autoHealAppCompanies(rootDbPath, dataDir) {
    ...
    if (fs.existsSync(dataDir)) {
        ...
        for (const siloId of silos) {
            try {
                const siloDb = new better_sqlite3_1.default(path.join(dataDir, siloId, 'active_db.sqlite'), { readonly: true });
                let profile = null;
                try {
                    const profileRow = siloDb.prepare("SELECT value FROM store WHERE key = 'app_company_profile'").get();
                    if (profileRow && profileRow.value)
                ...
                if (!profile) {
                    try {
                        const companyRow = siloDb.prepare("SELECT value FROM store WHERE key = 'app_companies'").get();
                        if (companyRow && companyRow.value) {
                            const comps = JSON.parse(companyRow.value);
                            profile = comps.find((c) => c.id === siloId) || comps[0];
                        }
                    }
                    catch (e) { }
                }
    ...
    if (recoveredCompanies.length > 0) {
        ...
        try {
            const writeDb = new better_sqlite3_1.default(rootDbPath);
...
function siloHasEmployees(siloDataPath, siloId) {
    ...
    if (!fs.existsSync(siloDbPath))
        return false;
    try {
        const siloDb = new better_sqlite3_1.default(siloDbPath, { readonly: true });
        const stmt = siloDb.prepare('SELECT value FROM store WHERE key = ?');
        const row = stmt.get(`app_employees_${siloId}`);
        siloDb.close();
        if (row && row.value) {
            try {
...
function readActivatedSilos() {
    try {
        const filePath = path.join(electron_1.app.getPath('userData'), 'sys_limit.bin');
        if (!fs.existsSync(filePath))
            return [];
        const encrypted = fs.readFileSync(filePath, 'utf8');
        const decipher = crypto.createDecipheriv('aes-256-cbc', LIMIT_KEY, LIMIT_IV);
...
function writeActivatedSilos(silos) {
    try {
        const filePath = path.join(electron_1.app.getPath('userData'), 'sys_limit.bin');
        const cipher = crypto.createCipheriv('aes-256-cbc', LIMIT_KEY, LIMIT_IV);
...
electron_1.ipcMain.handle('get-activated-silos', async () => {
    return { success: true, silos: readActivatedSilos() };
});
electron_1.ipcMain.handle('register-activated-silo', async (_, signature) => {
...
electron_1.ipcMain.handle('find-bpp-app', async () => {
    try {
        ...
        potentialRoots.push(path.join(electron_1.app.getPath('home'), 'BharatPayRoll'));
...
electron_1.ipcMain.handle('generate-diagnostics', async (_, uiState) => {
    try {
        if (!mainWindow)
            throw new Error("No main window");
        const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
        const defaultPath = path.join(electron_1.app.getPath('desktop'), `BPP_Diagnostics_${timestamp}.bpplog`);
                    ...
                    for (const item of items) {
                        if (item.isDirectory()) {
                            ...
                            if (fs.existsSync(siloDbPath) || item.name.includes('_')) {
                                ...
                                if (!fsState.silos.some((s) => s.folder === item.name)) {
                                    fsState.silos.push({
